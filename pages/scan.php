...
<body class="g-sidenav-show   bg-gray-100">
  ...
  <div class="col-md-6">
    <div class="card">
      <div class="card-body">
        <h5 class="card-title mb-4">Analyse Active </h5>
        <p class="card-text">
          - Fournit un rapport avancé et détaillé<br>
          - Prend plus de temps pour traiter
        </p>
        <form class="form-inline d-flex align-items-center" action="zap_integActive.php" method="post" onsubmit="disableAllFormElementsWithDelay()" >
          <div class="form-group mx-sm-3 mb-2">
            <label for="targetUrl" class="sr-only">Target URL:</label>
            <input class="form-control" placeholder="url..." type="text" id="targetUrl" name="targetUrlActive" required>
          </div>
          <input type="submit" class="btn btn-primary mb-2" value="Initiate Scan" >
        </form>
      </div>
    </div>
  </div>
  ...
  <div class="col-md-6">
    <div class="card">
      <div class="card-body">
        <h5 class="card-title mb-4">Analyse Spider</h5>
        <p class="card-text">
          - Fournit un rapport avancé et détaillé<br>
          - Prend plus de temps pour traiter
        </p>
        <form class="form-inline d-flex align-items-center" action="zap_integSpider.php" method="post" onsubmit="disableAllFormElementsWithDelay()">
          <div class="form-group mx-sm-3 mb-2">
            <label for="targetUrl" class="sr-only">Target URL:</label>
            <input class="form-control" placeholder="url..." type="text" id="targetUrl" name="targetUrlSpider" required>
          </div>
          <input type="submit" class="btn btn-primary mb-2" value="Initiate Scan">
        </form>
      </div>
...
<!-- disabling buttons after entering url -->
<script>
function disableAllFormElementsWithDelay() {
  setTimeout(() => {
    const inputs = document.querySelectorAll('input');
    inputs.forEach(input => {
      input.disabled = true;
    });
  }, 100); // 100 milliseconds delay to allow form submission
}
</script>
...
document.addEventListener('DOMContentLoaded', () => {
  const activeScanForm = document.getElementById('activeScanForm');
  const spiderScanForm = document.getElementById('spiderScanForm');

  activeScanForm.addEventListener('submit', (event) => {
    const targetUrl = document.getElementById('targetUrlActive').value.trim();
    const message = document.getElementById('activeScanMessage');
    if (targetUrl === '') {
  ...
  spiderScanForm.addEventListener('submit', (event) => {
    const targetUrl = document.getElementById('targetUrlSpider').value.trim();
    const message = document.getElementById('spiderScanMessage');
    if (targetUrl === '') {
      event.preventDefault();
      message.textContent = 'Please fill in the input.';
    } else {
