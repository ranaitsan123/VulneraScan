...
// Function to sanitize and validate URL input
function sanitize_url($url) {
  ...
  if (filter_var($url, FILTER_VALIDATE_URL)) {
      ...
      return "The URL format isn't valid.";
  }
}

// Configuration de l'API ZAP
$zapUrl = 'http://localhost:8081';
$apiKey = 'fdhei51923c0uc3lle1fpor3sq';


// Sanitize and filter the targetUrl input
$targetUrl = sanitize_url($_POST['targetUrlActive']);
 ...
 <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
  ...
  <style>
     ...
     .footer {
         ...
         bottom: 0;
         width: 100%;
         background-color: #f8f9fe; /* Change the background color as needed */
  ...
  <aside class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-4 " id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <!-- Scan section list -->
        <li class="nav-item">
          <a class="nav-link active" href="scan.php">
            <div class="icon icon-shape icon-sm border-radius-md text-center me-2 d-flex align-items-center justify-content-center">
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl " id="navbarBlur" data-scroll="false">
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          ...
          <h6 class="font-weight-bolder text-white mb-0">Scan Actif</h6>
        </nav>
      ...
      <div class="row mt-4">
         <div class="card">
             <?php 
              ...
              // Invalid URL, display appropriate error message
          switch ($targetUrl) {
            case "The URL is empty.":
                echo '<div class="d-flex justify-content-center align-items-center flex-column " style=" margin-bottom: 50px; margin-top: 50px;">';
                echo '<img src="../assets/img/oops_icon.png" alt="oops_icon" style="max-width: 40%; height: auto; opacity: 0.5; padding: 20px;">';
                echo '<h3 style="color: grey;"> Oops! </h3>';
                echo "L'URL est vide. Veuillez fournir une URL valide.";
                echo '</div>';
                break;
            case "The URL format isn't valid.":
                echo '<div class="d-flex justify-content-center align-items-center flex-column " style=" margin-bottom: 50px; margin-top: 50px;">';
                echo '<img src="../assets/img/oops_icon.png" alt="oops_icon" style="max-width: 40%; height: auto; opacity: 0.5; padding: 20px;">';
                echo '<h3 style="color: grey;"> Oops! </h3>';
                echo "Le format de l'URL n'est pas valide. Veuillez vérifier le format de l'URL.";
                echo '</div>';
                break;
            default:

            // Configuration de la cible à scanner
if (isset($_POST['targetUrlActive'])) {

     // Démarrage de la session ZAP
     $ch = curl_init();
     curl_setopt($ch, CURLOPT_URL, $zapUrl . '/JSON/core/action/newSession');
     curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
     curl_setopt($ch, CURLOPT_POST, true);
     curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(['apikey' => $apiKey]));
     $response = curl_exec($ch);
     curl_close($ch);
 
     // Démarrage du scan Spider
     $ch = curl_init();
     curl_setopt($ch, CURLOPT_URL, $zapUrl . '/JSON/spider/action/scan/');
     curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
     curl_setopt($ch, CURLOPT_POST, true);
     curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(['apikey' => $apiKey, 'url' => $targetUrl]));
     $spiderResponse = curl_exec($ch);
     curl_close($ch);
 
     // Attente de la fin du scan Spider (augmenter le délai si nécessaire)
     sleep(10);
 
     // Démarrage du scan actif
     $ch = curl_init();
     curl_setopt($ch, CURLOPT_URL, $zapUrl . '/JSON/ascan/action/scan/');
     curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
     curl_setopt($ch, CURLOPT_POST, true);
     curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(['apikey' => $apiKey, 'url' => $targetUrl, 'recurse' => 'true']));
     $ascanResponse = curl_exec($ch);
     curl_close($ch);
 
     // Vérification régulière de l'état du scan actif
     $scanActiveCompleted = false;
     while (!$scanActiveCompleted) {
         sleep(5); // Attendre 5 secondes avant chaque vérification
         $ch = curl_init();
         curl_setopt($ch, CURLOPT_URL, $zapUrl . '/JSON/ascan/view/status/');
         curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
         curl_setopt($ch, CURLOPT_POST, true);
         curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(['apikey' => $apiKey]));
         ...
         if (isset($status['status']) && $status['status'] == '100') {
             $scanActiveCompleted = true;
         }
     }
 
          // Récupération des alertes
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $zapUrl . '/JSON/core/view/alerts');
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_HTTPHEADER, ['X-ZAP-API-Key: ' . $apiKey]);
    $alertsResponse = curl_exec($ch);
    curl_close($ch);

    $alerts = json_decode($alertsResponse, true)['alerts'];

    // Préparation de la requête d'insertion
    $sql = "INSERT INTO history (timestamp, user_id, url, result, user_name, scan_type) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssss", $timestamp, $user_id, $targetUrl, $result, $uname, $scan_type);
    
    $scan_type = 'active';
    $timestamp = date('Y-m-d H:i:s');
    $user_id = $_SESSION['id'];
    $result = json_encode($alerts);
    ...
    if ($stmt->execute()) {
      ...
      echo "<p>Erreur lors de l'insertion : " . $stmt->error . "</p>";
      echo '</div>';
  }
  ...
  echo '<div class="alert error">';
  echo "<p>Aucune URL fournie pour le scan.</p>";
  echo '</div>';
}
