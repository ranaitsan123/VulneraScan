  ...
  <main class="main-content position-relative border-radius-lg ">
    ...
    <div class="container-fluid py-4">
      ...
      <div class="card">
        <div class="card-body">
        <?php
         $sql2 = "SELECT id, timestamp, user_name, url, result, scan_type FROM history WHERE user_name = '$uname'";
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<tr><th>Date/Heure</th><th>Utilisateur</th><th>URL</th><th>Resultat</th><th>Telecharger</th></tr>";
    while($row = $result->fetch_assoc()) {
        echo "<tr>";
        echo "<td>". htmlspecialchars($row["timestamp"])."<br><a href=\"javascript:;\" class=\"btn btn-primary btn-sm\">"./*"<span class="d-sm-inline d-none">".*/htmlspecialchars($row["scan_type"])/*."</span>"*/."</a>". "</td>";
        echo "<td>". htmlspecialchars($row["user_name"]). "</td>";
        echo "<td>". htmlspecialchars($row["url"]). "</td>";
            echo "<td><a href='view_result.php?id=" . $row["id"] . "' class='btn btn-primary btn-sm'> Afficher rapport</a></td>";
            echo "<td><a href='telechargement_pdf.php?id=" . $row["id"] . "' class='btn btn-link text-dark text-sm mb-0 px-0 ms-4' ><i class='fas fa-file-pdf text-lg me-1'></i> PDF</a></td>";
            echo "</tr>";
    }
        echo "</table>";
    } else {
        echo "<body style='background-color: #f8f8f8;'></body>";
        echo "<div style=\"font-family: 'Times New Roman'; font-size: 28px; font-weight: bold; \">";
        echo "Aucun scan n'a été effectué";
        echo "</div>";
    }
    echo "<script>";
    echo "function loadResult(id) {";
    echo "document.getElementById('resultsTable').style.display = 'none';"; // Hide the table
    echo "fetch('view_result.php?id=' + id).then(function(response) {";
    echo "return response.text();";
    echo "}).then(function(html) {";
    echo "document.getElementById('result_display').innerHTML = html;";
    echo "}).catch(function(err) {";
    echo "console.warn('Something went wrong.', err);";
    echo "});";
    echo "}";
    echo "</script>";
    echo "<div id='result_display'></div>";
    }
    ?>
        </div>
      </div>
